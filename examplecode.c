//skip ambient light in this project
Object** object;
for(int y=0; y<M; y+=1){
	for(int x = 0; x<N; x+=1){
		Ro = ...;
		...
		for(int j=0; light[j]) != NULL; j+=1){
			//Shadow test
			Ron = closest_t * Rd + Ro;		//vectors so have to use 3d operations. can't add/multiply
			...
			for(int k=0; object[k] != NULL; k +=1){
				...
				if(best_t > distance_to_light) continue;
			}
			if(closest_shadow_object == NULL){
				...
				R = reflection of L; //equaiton on 3d math slide (.product)
				V = Rd;
				
				diffuse = ...;  //uses objects diffuse color
		...
		buffer[...].r = (unsigned char)(255 * clamp(closest_object->color)); //clamp(color[0])
		buffer[...].g = (unsigned char)(255 * clamp(closest_object->color));
		buffer[...].b = (unsigned char)(255 * clamp(closest_object->color));
